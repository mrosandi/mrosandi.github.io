...
(function initFluid() {
  ...
  const bloomPreFrag = compileShader(
    gl.FRAGMENT_SHADER,
  ...
  const bloomBlurFrag = compileShader(
    gl.FRAGMENT_SHADER,
    `precision mediump float;precision mediump sampler2D;varying vec2 vL;varying vec2 vR;varying vec2 vT;varying vec2 vB;uniform sampler2D uTexture;void main(){vec4 s=vec4(0.);s+=texture2D(uTexture,vL);s+=texture2D(uTexture,vR);s+=texture2D(uTexture,vT);s+=texture2D(uTexture,vB);s*=.25;gl_FragColor=s;}`,
  );
  const bloomFinFrag = compileShader(
    gl.FRAGMENT_SHADER,
  ...
  const displayBloomFrag = compileShader(
    gl.FRAGMENT_SHADER,
    `precision highp float;precision highp sampler2D;varying vec2 vUv;uniform sampler2D uTexture;uniform sampler2D uBloom;void main(){vec3 C=texture2D(uTexture,vUv).rgb;vec3 bloom=texture2D(uBloom,vUv).rgb;bloom=pow(bloom.rgb,vec3(1./2.2));C+=bloom;float a=max(C.r,max(C.g,C.b));gl_FragColor=vec4(C,a);}`,
  );
  const splatFrag = compileShader(
    gl.FRAGMENT_SHADER,
    `precision highp float;precision highp sampler2D;varying vec2 vUv;uniform sampler2D uTarget;uniform float aspectRatio;uniform vec3 color;uniform vec2 point;uniform float radius;void main(){vec2 p=vUv-point.xy;p.x*=aspectRatio;vec3 splat=exp(-dot(p,p)/radius)*color;vec3 base=texture2D(uTarget,vUv).xyz;gl_FragColor=vec4(base+splat,1.);}`,
  );
  const advFrag = compileShader(
    gl.FRAGMENT_SHADER,
  ...
  const advManFrag = compileShader(
    gl.FRAGMENT_SHADER,
  ...
  function createDoubleFBO(w, h, inF, fmt, type, param) {
    let f1 = createFBO(w, h, inF, fmt, type, param),
      f2 = createFBO(w, h, inF, fmt, type, param);
    return {
      get read() {
        return f1;
      },
  ...
  function resizeFBO(target, w, h, inF, fmt, type, param) {
    ...
    clearProg.bind();
    gl.uniform1i(clearProg.uniforms.uTexture, target.attach(0));
    gl.uniform1f(clearProg.uniforms.value, 1);
    blit(n.fbo);
    return n;
  }
  ...
  function initFBOs() {
    ...
    for (let i = 0; i < config.BLOOM_ITERATIONS; i++) {
      let w = bRes.width >> (i + 1),
        h = bRes.height >> (i + 1);
      if (w < 2 || h < 2) break;
      bloomFBOs.push(
        createFBO(w, h, rgba.internalFormat, rgba.format, texType, filt),
      );
    }
  }
  ...
  function step(dt) {
    ...
    blit(curl.fbo);
    vortProg.bind();
    gl.uniform2f(vortProg.uniforms.texelSize, 1 / simW, 1 / simH);
    ...
    velocity.swap();
    divProg.bind();
    gl.uniform2f(divProg.uniforms.texelSize, 1 / simW, 1 / simH);
    ...
    pressure.swap();
    presProg.bind();
    gl.uniform2f(presProg.uniforms.texelSize, 1 / simW, 1 / simH);
    ...
    gradProg.bind();
    gl.uniform2f(gradProg.uniforms.texelSize, 1 / simW, 1 / simH);
    ...
    velocity.swap();
    advProg.bind();
    gl.uniform2f(advProg.uniforms.texelSize, 1 / simW, 1 / simH);
    if (!supportLinear)
      gl.uniform2f(advProg.uniforms.dyeTexelSize, 1 / simW, 1 / simH);
    ...
    gl.viewport(0, 0, dyeW, dyeH);
    if (!supportLinear)
      gl.uniform2f(advProg.uniforms.dyeTexelSize, 1 / dyeW, 1 / dyeH);
    gl.uniform1i(advProg.uniforms.uVelocity, velocity.read.attach(0));
    gl.uniform1i(advProg.uniforms.uSource, density.read.attach(1));
  ...
  function applyBloom(source, dest) {
    ...
    for (let i = 0; i < bloomFBOs.length; i++) {
      const d = bloomFBOs[i];
      gl.uniform2f(
        bloomBlurProg.uniforms.texelSize,
        1 / last.width,
        1 / last.height,
      );
    ...
    bloomFinProg.bind();
    gl.uniform2f(
      bloomFinProg.uniforms.texelSize,
      1 / last.width,
      1 / last.height,
    );
    gl.uniform1i(bloomFinProg.uniforms.uTexture, last.attach(0));
    gl.uniform1f(bloomFinProg.uniforms.intensity, config.BLOOM_INTENSITY);
  ...
  function render(target) {
    ...
    gl.viewport(0, 0, w, h);
    colorProg.bind();
    gl.uniform4f(colorProg.uniforms.color, 0, 0, 0, 1);
    blit(target);
    const prog = config.BLOOM ? displayBloom : displayProg;
    prog.bind();
    gl.uniform1i(prog.uniforms.uTexture, density.read.attach(0));
  ...
  window._fluidSplat = function (clientX, clientY, dx, dy) {
    ...
    ptr.down = true;
    ptr.moved = true;
    if (!ptr.color || Math.random() < 0.02) ptr.color = generateColor();
  };
  ...
  window.addEventListener("keydown", (e) => {
    if (e.key === " ") splatStack.push(parseInt(Math.random() * 12) + 5);
  });
})();
...
(function initCursor() {
  ...
  document.addEventListener("mousemove", (e) => {
    ...
    inGl(rawX, rawY)
      ? dot.classList.add("zone-gl")
      : dot.classList.remove("zone-gl");
  ...
  document.addEventListener(
    ...
    (e) => {
      ...
      if (window._fluidSplat)
        window._fluidSplat(rawX, rawY, rawX - prevX, rawY - prevY);
      prevX = rawX;
      prevY = rawY;
    },
    { passive: false },
...
void main(){
  ...
  function sz() {
    ...
    if (isMobile) {
      ...
      s = Math.min(Math.round(window.innerWidth * 0.5), 860);
    }
    return { w: s, h: s };
  }
  let { w, h } = sz();
  ...
  loader.load("src/assets/photo-1.png", (t) => {
    t.minFilter = THREE.LinearMipmapLinearFilter;
    t.magFilter = THREE.LinearFilter;
    t.wrapS = THREE.ClampToEdgeWrapping;
    t.wrapT = THREE.ClampToEdgeWrapping;
    uniforms.uLayerA.value = t;
  });
  loader.load("src/assets/photo-2.png", (t) => {
    t.minFilter = THREE.LinearMipmapLinearFilter;
    t.magFilter = THREE.LinearFilter;
    t.wrapS = THREE.ClampToEdgeWrapping;
    t.wrapT = THREE.ClampToEdgeWrapping;
    uniforms.uLayerB.value = t;
  });
  ...
  function toUV(ex, ey) {
    ...
    return {
      lx: Math.max(0, Math.min(1, (ex - r.left) / r.width)),
      ly: Math.max(0, Math.min(1, 1 - (ey - r.top) / r.height)),
    };
  }
  document.addEventListener("mousemove", (e) => {
    const { lx, ly } = toUV(e.clientX, e.clientY);
    mx = lx;
    my = ly;
  });
  document.addEventListener(
    "touchmove",
    (e) => {
      const { lx, ly } = toUV(e.touches[0].clientX, e.touches[0].clientY);
      mx = lx;
      my = ly;
    },
    { passive: false },
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // 1. Simulasi angka naik perlahan selama proses download berjalan
  const counterInterval = setInterval(() => {
    if (progress < 95) {
      progress += Math.floor(Math.random() * 5) + 1;
      if (progress > 95) progress = 95; // Tertahan di 95% jika internet lambat
      progressText.innerText = progress + "%";
    }
  }, 100);

  // 2. MAKSIMAL BERGANTUNG KONEKSI: Aktif tepat saat seluruh aset selesai diunduh
  window.addEventListener("load", () => {
    clearInterval(counterInterval); // Hentikan simulasi angka

    progressText.innerText = "100%"; // Set langsung ke 100%
